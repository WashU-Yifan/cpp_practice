...
HoldEmHandRank HoldEmGame::holdEmHandEvaluation(CardSet<HoldEmRank, Suit>& cardSet) {

    vector<Card_T<HoldEmRank, Suit> > evalHands(cardSet.getDeckBegin(), cardSet.getDeckEnd());
    for (auto cardIter = sharedHands.getDeckBegin(); cardIter != sharedHands.getDeckEnd(); ++cardIter){
        evalHands.push_back(*cardIter);
    }
...
void HoldEmGame:: printPlayers() {
    for (long unsigned int i = 0; i < playersHands.size(); ++i) {
        sort(playersHands[i].getDeckBegin(), playersHands[i].getDeckEnd(), rankCompare<HoldEmRank, Suit>);
        cout << players[i] << endl;
...
int HoldEmGame::play() {
    while(true){
        ...
        for (long unsigned int i = 0; i < playersHands.size(); ++i) {
            ...
            sort(playerRanks[i].cardSet.getDeckBegin(), playerRanks[i].cardSet.getDeckEnd(), rankCompare<HoldEmRank, Suit>);
...
bool operator<(const HoldEmGame::HoldEmHand& hand1, const HoldEmGame::HoldEmHand &hand2) {
    ...
    vector< Card_T<HoldEmRank, Suit> > deck1 (handCopy1.cardSet.getDeckBegin(), handCopy1.cardSet.getDeckEnd()),
    deck2(handCopy2.cardSet.getDeckBegin(), handCopy2.cardSet.getDeckEnd());
